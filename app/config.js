...
var pipeline = [
    ...
    function openShift(context) {
        if (process.env.OPENSHIFT_BUILD_NAME || process.env.OPENSHIFT_APP_NAME) {
            ...
            context.result.database.uri = "mongodb://"+process.env['DATABASE_USER']+":"+process.env['DATABASE_PASSWORD']+"@"+process.env.MONGODB_SERVICE_HOST+":"+process.env.MONGODB_SERVICE_PORT+"/"+process.env['DATABASE_NAME'];
        }
    }
];
